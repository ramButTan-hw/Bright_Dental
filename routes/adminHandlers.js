...
function createAdminHandlers(deps) {
  ...
  function getFinancialDetailReport(req, res) {
    ...
    if (patientSearch) { conditions.push("CONCAT(p.p_first_name, ' ', p.p_last_name) LIKE ?"); params.push(`%${patientSearch}%`); }
    if (doctorId) { conditions.push('a.doctor_id = ?'); params.push(doctorId); }
    if (locationId) { conditions.push('a.location_id = ?'); params.push(locationId); }
    if (statusGroup === 'COMPLETED') { conditions.push("UPPER(s.status_name) = 'COMPLETED'"); }
    else if (statusGroup === 'SCHEDULED') { conditions.push("UPPER(s.status_name) IN ('SCHEDULED', 'CONFIRMED', 'RESCHEDULED', 'CHECKED_IN')"); }
    else if (statusGroup === 'CANCELLED') { conditions.push("UPPER(s.status_name) IN ('CANCELED', 'CANCELLED')"); }
    else if (statusGroup === 'NO_SHOW') { conditions.push("UPPER(s.status_name) = 'NO_SHOW'"); }
    else if (statusGroup === 'MISSED') { conditions.push("UPPER(s.status_name) IN ('CANCELED', 'CANCELLED', 'NO_SHOW')"); }
    if (paymentStatus !== 'ALL') { conditions.push("UPPER(COALESCE(i.payment_status, '')) = ?"); params.push(paymentStatus); }
    if (patientState !== 'ALL') { conditions.push("UPPER(COALESCE(p.p_state, '')) = ?"); params.push(patientState); }

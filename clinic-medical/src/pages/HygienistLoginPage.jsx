//hygienist login page
import { useEffect, useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { jsPDF } from 'jspdf';
import autoTable from 'jspdf-autotable';
import { getHygienistPortalSession, resolveApiBaseUrl, setHygienistPortalSession } from '../utils/patientPortal';
import '../styles/HygienistDashboardPage.css';
...
function HygienistLoginPage() {
  const API_BASE_URL = useMemo(() => resolveApiBaseUrl(), []);
  const navigate = useNavigate();
  const [session, setSession] = useState(() => getHygienistPortalSession());
  ...
  const loadAllAppointments = async (staffId) => {
    if (!Number.isInteger(staffId) || staffId <= 0) {
      setAllAppointments([]);
      return [];
    }
    const response = await fetch(`${API_BASE_URL}/api/hygienist/appointments?staffId=${staffId}`);
  ...
  const refreshAppointments = async (staffId, dateValue) => {
    const safeDate = /^\d{4}-\d{2}-\d{2}$/.test(String(dateValue || ''))
      ? String(dateValue)
      : getLocalDateString();
    const allAppts = await loadAllAppointments(staffId);
  ...
  useEffect(() => {
    const activeSession = getHygienistPortalSession();
    ...
    const load = async () => {
      const profileUrl = activeSession.userId
        ? `${API_BASE_URL}/api/hygienist/profile?userId=${activeSession.userId}`
        : `${API_BASE_URL}/api/hygienist/profile-by-username?username=${encodeURIComponent(activeSession.username || '')}`;
      const [profileRes] = await Promise.all([
        fetch(profileUrl)
      ]);
      const profilePayload = await profileRes.json().catch(() => ({}));
      if (!profileRes.ok) {
        throw new Error(profilePayload.error || 'Failed to load hygienist profile');
      }
      ...
      const updatedSession = {
        ...
        username: activeSession.username || profilePayload?.user_username || 'hygienist',
        fullName: activeSession.fullName || [profilePayload?.first_name, profilePayload?.last_name].filter(Boolean).join(' ').trim() || 'Hygienist'
      };
      setHygienistPortalSession(updatedSession);
      setSession(updatedSession);
      await refreshAppointments(Number(updatedSession.staffId), selectedDate);
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      refreshAppointments(Number(session.staffId), selectedDate)
        .catch((err) => setMessage(err.message || 'Unable to refresh appointments for selected date.'));
    }, 0);

    return () => window.clearTimeout(timeoutId);
  }, [session?.staffId, selectedDate]);

  const welcomeName = profile?.first_name || session?.fullName || 'Hygienist';
  const missingItems = [
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/hygienist/patients/appointments/search?staffId=${session.staffId}&query=${encodeURIComponent(query)}`,
  ...
  }, [API_BASE_URL, patientSearchTerm, session?.staffId]);
  ...
  const generateSinglePatientReport = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/hygienist/reports/patient?${queryString}`);
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
      ...
      exportReportPayload(
        payload,
        singleReportFormat,
        `hygienist-patient-${patientId}-report-${singleReportForm.fromDate}-to-${singleReportForm.toDate}`,
  ...
  const generateMultiPatientReport = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/hygienist/reports/patients?${queryString}`);
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
      ...
      exportReportPayload(
        payload,
        multiReportFormat,
        `hygienist-multi-patient-report-${multiReportForm.fromDate}-to-${multiReportForm.toDate}`,
    ...
    return (
      <main className="dentist-page">
        <section className="dentist-panel">
          <h1>Hygienist Page</h1>
          <p className="dentist-subtle">No hygienist session was found. Please sign in through Staff Login.</p>
  ...
  return (
    <main className="dentist-page">
      <section className="dentist-hero">
        <div>
          <p className="dentist-subtle">Hygienist Workspace</p>
                ...
                return (
                  <li key={appt.appointment_id}>
                    <button
                      ...
                      onClick={() => isCheckedIn && navigate(`/hygienist/patient/${appt.appointment_id}`)}
                    >
                      <div>
                ...
                <li key={appt.appointment_id}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (Number(appt.staff_id) === Number(session?.staffId)) {
                        navigate(`/hygienist/patient/${appt.appointment_id}`);
                      }
                    }}
                  ...
                  >
                    <div className="dentist-result-info">
                      ...
                      {Number(appt.staff_id) !== Number(session?.staffId) && (
                        <div className="dentist-result-date">Read-only result (assigned to another hygienist).</div>
                      )}
                    </div>
                  </button>
                </li>
                ...
                <li key={appt.appointment_id} className="dentist-past-item">
                  ...
                  <div className="dentist-past-patient dentist-past-patient--inline">
                    <button
                      ...
                      onClick={() => navigate(`/hygienist/patient/${appt.appointment_id}`)}
                    >
                      {appt.patient_name}
                    </button>
                  </div>
...
export default HygienistLoginPage;

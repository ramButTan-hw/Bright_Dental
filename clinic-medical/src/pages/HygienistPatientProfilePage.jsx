...
import { formatDate, formatTime, getHygienistPortalSession, resolveApiBaseUrl } from '../utils/patientPortal';
...
const FALLBACK_ADA_CODES = [
  ...
  { procedure_code: 'D0210', description: 'Full-mouth radiographic survey', default_fees: 180 },
  { procedure_code: 'D1110', description: 'Adult prophylaxis (cleaning)', default_fees: 140 },
  { procedure_code: 'D1120', description: 'Child prophylaxis (cleaning)', default_fees: 95 },
  { procedure_code: 'D1206', description: 'Topical fluoride varnish', default_fees: 55 },
  { procedure_code: 'D4341', description: 'Periodontal scaling and root planing (4+ teeth, per quadrant)', default_fees: 280 },
  { procedure_code: 'D4342', description: 'Periodontal scaling and root planing (1-3 teeth, per quadrant)', default_fees: 245 },
  { procedure_code: 'D4910', description: 'Periodontal maintenance', default_fees: 175 }
];
...
function HygienistPatientProfilePage() {
  const { appointmentId } = useParams();
  const navigate = useNavigate();
  const API_BASE_URL = useMemo(() => resolveApiBaseUrl(), []);
  const session = useMemo(() => getHygienistPortalSession(), []);
  ...
  const staffId = Number(session?.staffId || 0);

  const loadDetail = async () => {
    if (!staffId || !appointmentId) {
      setError('Missing hygienist session or appointment id.');
      setLoading(false);
      return;
    }
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE_URL}/api/hygienist/appointments/${appointmentId}?staffId=${staffId}`);
  ...
  }, [API_BASE_URL, appointmentId, staffId]);

  useEffect(() => {
    const loadProcedureCodes = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/hygienist/ada-procedure-codes`);
        const payload = await response.json().catch(() => []);
        if (!response.ok) {
  ...
  const saveVisitEntries = async (event) => {
    ...
    try {
      ...
      if (hasFindings) {
        ...
        const findingResponse = await fetch(`${API_BASE_URL}/api/hygienist/appointments/${appointmentId}/dental-findings?staffId=${staffId}`, {
          method: 'POST',
      ...
      const treatmentResponse = await fetch(`${API_BASE_URL}/api/hygienist/appointments/${appointmentId}/treatments?staffId=${staffId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (!detail?.patientProfile) {
    return (
      <main className="dentist-page">
        ...
        <button type="button" className="dentist-save-btn" onClick={() => navigate('/hygienist-login')}>Back to Appointments</button>
      </main>
    );
  }
  ...
  const deleteTreatmentEntry = async (planId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/hygienist/treatments/${numericPlanId}?staffId=${staffId}`, {
        method: 'DELETE'
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
  ...
  const deleteFindingEntry = async (findingId) => {
    ...
    try {
      const primaryResponse = await fetch(`${API_BASE_URL}/api/hygienist/findings/${numericFindingId}?staffId=${staffId}`, {
        method: 'DELETE'
      });
      ...
      if (!response.ok && response.status === 404 && appointmentId) {
        const fallbackResponse = await fetch(`${API_BASE_URL}/api/hygienist/appointments/${appointmentId}/findings/${numericFindingId}?staffId=${staffId}`, {
          method: 'DELETE'
        });
  ...
  const saveEditTreatment = async () => {
    ...
    setMessage('');
    try {
      const response = await fetch(`${API_BASE_URL}/api/hygienist/treatments/${editingTreatment.planId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          staffId,
  ...
  return (
    <main className="dentist-page">
      <section className="dentist-hero">
        ...
        <Link to="/hygienist-login" className="dentist-save-btn" style={{ textDecoration: 'none' }}>Back to Appointments</Link>
      </section>
...
export default HygienistPatientProfilePage;

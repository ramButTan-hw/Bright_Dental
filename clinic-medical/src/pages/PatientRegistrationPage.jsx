...
function PatientRegistrationPage() {
  ...
  const checkEmailExists = async (email) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/patients/check-email`, {
        method: 'POST',
  ...
  const handleIdentitySubmit = async (e) => {
    ...
    try {
      const emailTaken = await checkEmailExists(identity.email.trim());
      if (emailTaken) {
      ...
      setIdentityError('We could not verify your email right now. Please try again.');
    } finally {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitSuccess('Registration successful! We will be in touch shortly.');
      setStep(1);
      ...
      navigate('/patient-login', {
        state: {
          ...
          appointmentConfirmation: payload.appointmentConfirmation || null
        }
      });
    } catch (error) {
  ...
  return (
    <main className="patient-registration-page">
      <section className="registration-hero">
        <p className="registration-label">New Patient Intake</p>
        <h1>Book Your Appointment</h1>
        <p>
          Complete this quick registration and our front desk team will contact you
          to confirm your preferred appointment time.
        </p>
      </section>

      <section className="registration-form-section">
        <form className="registration-form" onSubmit={step === 1 ? handleIdentitySubmit : handleSubmit}>
          ...
          <div className="form-actions">
            {step > 1 && <button type="button" onClick={() => setStep((prev) => prev - 1)} className="back-btn">← Back</button>}
            <button type="submit" className="submit-btn" disabled={checkingEmail || submittingRegistration || appointmentStepInvalid}>
              {step === 1
                ? checkingEmail
                  ? 'Checking Email...'
                  : 'Continue'
          ...
          <p className="existing-account-link-wrap">
            <Link to="/patient-login" className="existing-account-link">
              Click here if you are already part of Bright!
            </Link>
          </p>
        </form>
      </section>
      ...
      {showExistingPatientModal && (
        <div className="modal-overlay" role="dialog" aria-modal="true">
          <div className="modal-card">
            ...
            <p>
              It looks like you may already be part of Bright Dental. Would you like
              to log in?
            </p>
            <div className="modal-actions">
              <Link to="/patient-login" className="modal-login-btn">

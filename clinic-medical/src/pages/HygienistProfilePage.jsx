...
import { getHygienistPortalSession, resolveApiBaseUrl } from '../utils/patientPortal';
...
function HygienistProfilePage() {
  const API_BASE_URL = useMemo(() => resolveApiBaseUrl(), []);
  const navigate = useNavigate();
  const session = getHygienistPortalSession();
  ...
  useEffect(() => {
    ...
    fetchWithTimeout(`${API_BASE_URL}/api/hygienist/profile?userId=${session.userId}`)
      .then(async (res) => {
  ...
  }, [API_BASE_URL, navigate, session?.userId]);
  ...
  useEffect(() => {
    ...
    const loadTimeOffData = async () => {
      const [requestRows, locationRows] = await Promise.all([
        fetchWithTimeout(`${API_BASE_URL}/api/staff/time-off-requests?staffId=${encodeURIComponent(resolvedStaffId)}`).then((r) => r.json().catch(() => [])),
  ...
  const updateField = (field, value) => {
    if (field === 'emergencyContactPhone') {
      setForm((prev) => ({ ...prev, [field]: formatEmergencyPhone(value) }));
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await fetchWithTimeout(`${API_BASE_URL}/api/hygienist/profile?userId=${session.userId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <main className="dentist-profile-page">
      <section className="dentist-profile-panel">
        <h1>Update Personal Information</h1>
        <p className="dentist-profile-subtle">Use this page to keep your contact information current.</p>
          ...
          <div className="dentist-profile-form-container">
            <form className="dentist-profile-form" onSubmit={handleSubmit}>
              ...
              <div className="dentist-profile-actions">
                <button type="button" className="dentist-profile-secondary" onClick={() => navigate('/hygienist-login')}>Back to Dashboard</button>
          ...
          <form className="dentist-profile-form dentist-profile-timeoff-form" onSubmit={submitTimeOffRequest}>
            ...
            <label>
              ...
              >
                ...
                {CLINIC_TIME_SELECT_OPTIONS.map((timeOption) => (
                  <option key={`hygienist-profile-start-${timeOption.value}`} value={timeOption.value}>
                    {timeOption.label}
                  </option>
                ))}
              </select>
            ...
            <label>
              ...
              >
                ...
                {CLINIC_TIME_SELECT_OPTIONS.map((timeOption) => (
                  <option key={`hygienist-profile-end-${timeOption.value}`} value={timeOption.value}>
                    {timeOption.label}
                  </option>
                ))}
              </select>
...
export default HygienistProfilePage;
